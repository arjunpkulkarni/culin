...
if TYPE_CHECKING:
    ...
    from layer1_app.services.lookup import LookupTables
...
class NutrientCalculator:
    ...
    def __init__(
        ...
        """Initialize calculator. Uses lookup_tables when provided (no DB); else uses db Session."""
        self.db = db
        self._lookup = lookup_tables
        self._nutrient_cache: Dict[int, Any] = {}
        self._cooking_method_cache: Dict[str, Any] = {}
    ...
    def calculate_recipe(
        ...
        if self._lookup:
            return self._calculate_recipe_lookup(parsed_ingredients, totals, audit_trail, cooking_method)
    ...
    def _calculate_recipe_lookup(
        ...
        """Fast path using in-memory nutrient profiles. Applies retention by cooking method when provided."""
        cooking_methods = getattr(self._lookup, "cooking_methods", {}) or {}
        key_to_nutrient_id = getattr(self._lookup, "key_to_nutrient_id", {}) or {}
        cooking_method_id = None
        if cooking_method and cooking_methods:
            cooking_method_id = cooking_methods.get(cooking_method.strip().lower())
        ...
        for ingredient in parsed_ingredients:
            if not ingredient.ingredient_id:
                ...
                continue
            profile = self._lookup.nutrient_profiles.get(ingredient.ingredient_id)
            if not profile:
                logger.warning(
                    f"No nutrient profile for ingredient {ingredient.ingredient_name}"
                )
                continue
            for key, amount_per_100g in profile.items():
                if key.startswith("other_"):
                    retention = self.DEFAULT_RETENTION
                elif cooking_method_id is not None:
                    nutrient_id = key_to_nutrient_id.get(key)
                    retention_factors = getattr(self._lookup, "retention_factors", {}) or {}
                    retention = (
                        retention_factors.get((nutrient_id, cooking_method_id))
                        if nutrient_id is not None else None
                    )
                ...
                final_contribution = raw_contribution * retention
                if key.startswith("other_"):
                    totals[key] = totals.get(key, 0.0) + final_contribution
                else:
                    totals[key] += final_contribution
                audit_trail.append(NutrientContribution(
                    ingredient_name=ingredient.ingredient_name,
                    mass_g=ingredient.mass_g,
                    nutrient_name=key,
                    nutrient_unit="g" if "g" in key else "mg" if "mg" in key else "mcg" if "mcg" in key else "",
                    raw_contribution=raw_contribution,
                    retention_factor=retention,
                    final_contribution=final_contribution,
                    source_fdc_id=None,
                ))
        return self._build_nutrient_totals(totals), audit_trail
    
    def _get_nutrient_profile(
        self,
        ingredient_id: int,
        cooking_method: Optional[Any]
        ...
        """
        from layer1_app.db.models import USDAFood, FoodNutrient
    ...
    def _get_retention_factor(
        ...
        from layer1_app.db.models import RetentionFactor
    ...
    def _get_nutrient(self, nutrient_id: int) -> Any:
        """Get nutrient from cache or database."""
        from layer1_app.db.models import Nutrient
    ...
    def _get_cooking_method(self, method_name: str) -> Optional[Any]:
        """Get cooking method from cache or database."""
        from layer1_app.db.models import CookingMethod
