...
1. Set environment variables: `DATABASE_URL` (PostgreSQL connection string, e.g. AWS RDS), `SECRET_KEY` (any string; required by Layer 1 config).
...
**Layer 3 (real):** Either (1) run the in-repo script to build **minimal placeholder** artifacts (good for local/dev), or (2) build full artifacts with the Layer 3 notebook (`layer3_artifact_build.ipynb`) and copy into `artifacts/layer3/`. Requires `numpy` (in main `requirements.txt`).

### Building Layer 3 artifacts (minimal, in-repo)

From `nutrition_engine/` with your venv activated (so `numpy` is available):

```bash
python scripts/build_layer3_artifacts.py
```

This writes `ingredient_embeddings.pkl`, `dish_embeddings.pkl`, `neighbor_index.pkl`, `macro_delta_stats.json`, and `confidence_params.json` into `artifacts/layer3/`. The app will then load Layer 3 at startup and run similarity refinement. For production-quality refinement, build artifacts from the [Layer 3 repo](https://github.com/vedaankb/CulinAIAPP-Layer3) and replace these files.

## Local test (run layer flow with your own data)

**Python version:** Use **Python 3.11 or 3.12**. Python 3.14 is not supported (spacy/Layer 1 dependencies fail). Check with `python3 --version`; if you see 3.14, install 3.11 or 3.12 (e.g. `brew install python@3.12` or [pyenv](https://github.com/pyenv/pyenv)) and use that to create the venv below.

From the project root or `nutrition_engine/`:

```bash
cd nutrition_engine

# Use Python 3.11 or 3.12 (not 3.14). Example with 3.12:
python3.12 -m venv .venv
# Or if python3 is already 3.11/3.12:  python3 -m venv .venv
...
Ensure `.env` in `nutrition_engine/` has `DATABASE_URL` and `SECRET_KEY` for real Layer 1 (otherwise Layer 1 uses a stub with zero macros).

```bash
python3 run_local_test.py
python3 run_local_test.py "Chicken burrito" "Grilled chicken, rice, beans"
python3 run_local_test.py --json '{"item_name": "Big Mac", "description": "Beef patties, bun", "restaurant": "McDonald'\''s", "price": 5.99}'
```

The script runs startup (load L2/L3 artifacts), then the full pipeline, and prints the same result as `POST /estimate`.
...
- **GET /food/search**, **POST /food/log** — FatSecret proxy (search and log food).

**Frontend:** See **docs/FRONTEND_API.md** for base URL, CORS, request/response shapes, and example `fetch` calls to connect your app.
